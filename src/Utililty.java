...
//			}
	 ...
	 * @return The list of all candidate documents.
	 */
	public static ArrayList<Integer> getListOfCandidates_(ArrayList<Integer> docIDsForWord, BitSet bloomFilterToCheckAgainst){
		
		ArrayList<Integer> candidateDocIDList = new ArrayList<Integer>();
		
		if(bloomFilterToCheckAgainst == null){
			return candidateDocIDList;
		}
		
		for(Integer docID : docIDsForWord) {
			BigInteger bigInt = BigInteger.valueOf(docID);
			
			int hashValue = (int) jenkinsHash.hash(bigInt.toByteArray()) % Term.HASH_SIZE;
			
			if(bloomFilterToCheckAgainst.get(hashValue)){
				candidateDocIDList.add(docID);
			}
		}
	 ...
	 * @return the list of valid document IDs.
	 */
	public static ArrayList<Integer> checkListOfCandidatesForReal_(ArrayList<Integer> candidateList, BitSet wordListToCheckAgainst){
		ArrayList<Integer> docIDList = new ArrayList<Integer>();
		for(Integer id : candidateList){
			if(wordListToCheckAgainst.get(id)){
				docIDList.add(id);
			}
		}
		return docIDList;
	 ...
	 * @return A list of intersection DocIDs.
	 */
	public static BitSet getListOfRealDocuments_(BitSet word1List, BitSet word2List){

		BitSet result = (BitSet) word1List.clone();
		result.and(word2List);
